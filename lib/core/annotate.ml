...
type _ Effect.t +=
  ...
  | Ctx : (('b, 's) status * ('a, 'b) Ctx.t) located -> unit Effect.t
...
let ctx status ctx = Effect.perform (Ctx (locate_opt (get_loc ()) (status, ctx)))

open Effect.Deep

type ctx_handler = { handle : 'a 'b 's. (('b, 's) status * ('a, 'b) Ctx.t) located -> unit }
type tm_handler = printable located -> unit
type ty_handler = printable located -> unit

let trivial_ctx_handler : ctx_handler = { handle = (fun _ -> ()) }
