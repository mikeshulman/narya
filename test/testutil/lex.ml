...
module Parse_tokens = struct
  module Basic = Token_parser.Make (Unit) (Token) (Token_list) (Unit)
  open Basic

  let token : Token.t t = step "token" (fun state _ tok -> Some (tok, state))
...
module Lex_and_parse =
  Parse_with_lexer.Make (Unit) (Token) (Token_list) (Unit) (Lexer.Parser) (Parse_tokens.Parser)
