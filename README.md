...
- A nonempty string consisting of the characters `[ ] ~ ! @ # $ % ^ & * / ? = + \ | , < > : ; - '` is always treated as a single token, and does not need to be surrounded by whitespace.  Note that this is all the non-alphanumeric characters that appear on a standard US keyboard except for those with other meanings: parentheses (grouping), curly braces (structures and, later, implicit arguments), backquote (comments), period (field access), and underscore (later, inferred arguments).  In particular:
...
Internal parametricity is implemented by the constant `Gel`, defined with `Narya.Gel.install ()`, whose type is
```
