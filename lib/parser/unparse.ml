...
let parenthesize tm = outfix ~notn:parens ~ws:[] ~inner:(Snoc (Emp, Term tm))
 ...
 fun notn args li ri ->
  ...
  | Open _, Open _ -> (
      ...
      | Some (first, Snoc (inner, last)) -> (
          ...
          | _ ->
              ...
              parenthesize (infix ~notn ~ws:[] ~first ~inner ~last ~left_ok ~right_ok))
  ...
  | Closed, Open _ -> (
      ...
      | Snoc (inner, last) -> (
          ...
          | Some right_ok ->
              let last = last.unparse (interval_right notn) ri in
              prefix ~notn ~ws:[] ~inner ~last ~right_ok
          | _ ->
              let last = last.unparse (interval_right notn) Interval.entire in
              let right_ok = No.minusomega_le t in
              parenthesize (prefix ~notn ~ws:[] ~inner ~last ~right_ok))
  ...
  | Open _, Closed -> (
      ...
      | Some (first, inner) -> (
          ...
          | Some left_ok ->
              let first = first.unparse li (interval_left notn) in
              postfix ~notn ~ws:[] ~first ~inner ~left_ok
          | _ ->
              let first = first.unparse Interval.entire (interval_left notn) in
              let left_ok = No.minusomega_le t in
              parenthesize (postfix ~notn ~ws:[] ~first ~inner ~left_ok))
...
let rec unparse :
 ...
 fun vars tm li ri ->
  ...
  | UU n ->
      unparse_act vars
        { unparse = (fun _ _ -> outfix ~notn:universe ~ws:[] ~inner:Emp) }
        (deg_zero n) li ri
  | Inst (ty, tyargs) ->
 ...
 fun cube vars xs body li ri ->
  ...
  | _ -> (
      ...
      | Some left_ok, Some right_ok ->
          ...
          infix ~notn ~ws:[] ~first ~inner:Emp ~last ~left_ok ~right_ok
      | _ ->
          let first =
          ...
          let right_ok = No.le_refl No.minus_omega in
          parenthesize (infix ~notn ~ws:[] ~first ~inner:Emp ~last ~left_ok ~right_ok))
...
and unparse_arrow :
 ...
 fun dom cod li ri ->
  ...
  | Some left_ok, Some right_ok ->
      ...
      infix ~notn:arrow ~ws:[] ~first ~inner:Emp ~last ~left_ok ~right_ok
  | _ ->
      ...
      parenthesize (infix ~notn:arrow ~ws:[] ~first ~inner:Emp ~last ~left_ok ~right_ok)
