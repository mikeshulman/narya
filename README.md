...
On the other hand, solving a hole changes the text of the Emacs buffer, and therefore it *can* be un-done with *Emacs's* `undo` command (generally bound to `C-/`, `C-_`, and `C-x u`), replacing the inserted term with the original `?`.  Since the "solve" command cannot be undone by Narya, if you undo it in Emacs there is no consistent thing that Narya can do with the command containing that hole.  Thus, in this case the Narya ProofGeneral mode automatically also retracts the processed region past the command containing the hole.

By default, when filling a hole interactively with ProofGeneral, the term you enter is automatically *reformatted*.  This primarily means that line breaks and indenting spaces are inserted in (what Narya thinks are) appropriate places (and removed from what it thinks are inappropriate places), and ASCII operators such as `->` and `|->` are replaced by their Unicode equivalents such as → and ↦.  You can customize the reformatting with the command-line flags `-compact` and `-noncompact` (insert fewer or more linebreaks, respectively) and `-unicode` and `-ascii` (print operators as → or `->`, respectively).  You can also turn off reformatting entirely by setting the Emacs customization variable `narya-reformat-holes` to `nil`.  However, if you don't like the way Narya reformats your terms, I would appreciate it if you give me feedback about it rather than (or, at least, in addition to) turning it off; the reformatter is experimental but I would like to improve it.
