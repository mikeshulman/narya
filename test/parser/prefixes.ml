...
let () =
  assert (parse prefixes "@ f x" = App (Notn ("at", [ Term (Ident [ "f" ]) ]), Ident [ "x" ]))

(* Since @ is a prefix notation, it can appear anywhere as an argument, so "f @ x" is parsed as "f (@ x)", and "f @ x y" is parsed as "f (@ x) y". *)
let () =
  assert (parse prefixes "f @ x" = App (Ident [ "f" ], Notn ("at", [ Term (Ident [ "x" ]) ])))

let () =
  assert (
    parse prefixes "f @ x y"
    = App (App (Ident [ "f" ], Notn ("at", [ Term (Ident [ "x" ]) ])), Ident [ "y" ]))
...
let () =
  assert (parse prefixes "@ @ x" = Notn ("at", [ Term (Notn ("at", [ Term (Ident [ "x" ]) ])) ]))
...
let () =
  assert (parse prefixes "f x ?" = Notn ("query", [ Term (App (Ident [ "f" ], Ident [ "x" ])) ]))
...
let () =
  assert (parse prefixes "f ! x" = App (Notn ("bang", [ Term (Ident [ "f" ]) ]), Ident [ "x" ]))

let () =
  assert (parse prefixes "f ? x" = App (Notn ("query", [ Term (Ident [ "f" ]) ]), Ident [ "x" ]))
