...
and eval : type m b s. (m, b) env -> (b, s) term -> s evaluation =
 fun env tm ->
  ...
  | Meta (meta, ambient) -> (
      ...
      | Wrap (Metadef { data = Def_meta None; _ }), Potential -> Unrealized
      ...
      | Wrap (Metadef { data = Def_meta None; ty; _ }), Kinetic ->
          Val (Uninst (Neu { head; args = Emp; value = ready Unrealized }, lazy (make_ty meta ty)))
      (* If a metavariable has a definition that fits with the current energy, we simply evaluate that definition. *)
      | Wrap (Metadef { data = Def_meta (Some tm); energy = Potential; _ }), Potential ->
          eval env tm
      | Wrap (Metadef { data = Def_meta (Some tm); energy = Kinetic; _ }), Kinetic -> eval env tm
      | Wrap (Metadef { data = Def_meta (Some tm); energy = Kinetic; _ }), Potential ->
          Realize (eval_term env tm)
      | Wrap (Metadef { data = Def_meta (Some tm); energy = Potential; ty; _ }), Kinetic -> (
          if GluedEval.read () then
