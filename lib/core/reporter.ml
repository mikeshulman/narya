...
module Code = struct
  type t =
    ...
    | Comatching_at_degenerated_codata : Constant.t -> t
    | No_such_constructor :
  ...
  let default_text : t -> text = function
    ...
    | Checking_tuple_at_degenerated_record r ->
        textf "can't check a tuple against a record %s with a nonidentity degeneracy applied"
          (name_of r)
    | Comatching_at_degenerated_codata r ->
        textf "can't comatch against a codatatype %s with a nonidentity degeneracy applied"
          (name_of r)
